...
while True:
    ...
    if answer == "y" or answer == "Y":
        ...
        while True:
            ...
            flag , img = cv2.VideoCapture('http://192.168.86.20:8080/video').read()
...
while True:
    ...
    answer = str(input())
    ret , img = cv2.VideoCapture('http://192.168.86.20:8080/video').read()
...
while 1:
    ...
    if board.turn == False and board.is_checkmate() == False:
        ...
        ret , img_1 = cv2.VideoCapture('http://192.168.86.20:8080/video').read()
        ...
        ret , img_2 = cv2.VideoCapture('http://192.168.86.20:8080/video').read()
        img_2 =   cv2.resize(img_2,(800,800))
        img_2 = get_warp_img(img_2,dir_path,img_resize)
        # cv2.imshow("Img_2 : ",img_2)
        br=False
        while True:
            try: move_word,game_img,flag = find_current_past_position(img_1,img_2,boxes,bool_position,board.fen(),chess_board,number_to_position_map,map_position); break
            except: print("Black hasn't moved")
            print("Player time to move : ")
            print("press 'q' when player moved ")
            while True:
                if cv2.waitKey(1) == ord('q'):
